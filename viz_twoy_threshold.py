...
from plot_distance_length_twoy_threshold import create_plot
...
with open("viz_twoy_threshold.yml", "r") as ymlfile:
...
if config["DISTANCE"]["plot_distance"]:
    ...
    if fin_distance:
        ...
        df_threshold = read_file(fin_distance, "Sheet2")
        st.info(
...
if (config["DISTANCE"]["plot_distance"] and df_distance is not None) or (
    ...
    if df_distance is not None:
        ...
        st.sidebar.write("% per timestamp")
        cmap_plot = st.sidebar.empty()
        config["DISTANCE"]["colormap_min"] = 0.
        config["DISTANCE"]["colormap_max"] = 100.
