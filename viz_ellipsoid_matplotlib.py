...
from ellipsoid import  plot_ellipsoid_matplotlib, get_ellipsoid_volume, get_min_vol_ellipse
...
if data_complete:
    ...
    with st.beta_expander("Visualization", expanded=True):
        ...
        col_data.write(pd.DataFrame(ellipsoids[time]["pos"], columns=["x", "y", "z"]))
        col_data.write(f"Volume: {ellipsoids[time]['volume']}")
        with st.spinner("Creating plot..."):
            ...
            fig = plt.figure()
            fig, (minx, miny, minz, maxx, maxy, maxz) = plot_ellipsoid_matplotlib(
                fig,
                ellipsoids[time]["pos"],
            ...
            outfile = col1.text_input(
                ...
                value=os.path.join(os.path.abspath(os.getcwd()), "ellipsoid.svg"),
            )
