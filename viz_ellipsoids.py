...
from ellipsoid import get_ellipsoid_volume, get_min_vol_ellipse
...
st.title("Ellipsoid Volumes Plot")
st.sidebar.markdown(
...
with st.spinner("Computing volumes..."):
    ...
    for df, data_name in zip(dfs, data_names):
        ...
        try:
            ...
            print(e)
            st.error("Unable to create ellipsoids from data, please check your data")
            st.stop()
...
with st.sidebar.beta_expander("Edit volume plot", expanded=False):
    ...
    if config is None:
        with open("viz_ellipsoids.yml") as fin_config:
    ...
    for i in range(len(data_names)):
        ...
        config["line_styles"][i] = st.selectbox(
            "Line style:", LINE_STYLES, index=line_style_idx,
            key = f"line_style{i}"
        )
        config["line_widths"][i] = st.number_input(
        ...
        config["markers"][i] = st.text_input("Marker type", value=config["markers"][i], key=f"marker{i}")
        config["marker_sizes"][i] = st.number_input(
